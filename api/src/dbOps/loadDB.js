...
for (let index = 0; index < 2; index++) {
  ...
  if (dom === "file://LangTechDom.xml") {
    ...
    CALL apoc.load.xml(${comp},'//CompetencyDetails/Competency',{},true) YIELD value as Competency
	  ...
	  c.link = "http://lingtransoft.info/competencies"
...
`);
	...
	MERGE (grp8)-[:GROUP_HAS_COMPETENCIES_OF {order: 27, planId: "2"}]->(cmp16:Competency {id: "Trans-GUID16"})-[:IS_IN_GROUP {order: 27, planId: "2"}]->(grp8)
	SET cmp16.label = "Principles",
		cmp16.default_weight = 1.0
	MERGE (cmp16)-[:HAS_SHORT_NAME]->(sn27:ShortName {id : "sn27"})
	SET sn27.label = "Principles"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp16)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp16)-[:HAS_TARGET_COMPETENCY]->(tc18:TargetCompetency {id : "tc18"})
	SET tc18.label = "Demonstrate a good understanding of translation principles (accurate, clear, natural, acceptable) and of translation theory and its application in translation. "
	...
	//Comp		Trans-GUID17 in Trans-Proc-Problems
	MERGE (grp8)-[:GROUP_HAS_COMPETENCIES_OF {order: 28, planId: "2"}]->(cmp17:Competency {id: "Trans-GUID17"})-[:IS_IN_GROUP {order: 28, planId: "2"}]->(grp8)
	SET cmp17.label = "Theological awareness",
		cmp17.default_weight = 1.0
	MERGE (cmp17)-[:HAS_SHORT_NAME]->(sn28:ShortName {id : "sn28"})
	SET sn28.label = "Theological awareness"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp17)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp17)-[:HAS_TARGET_COMPETENCY]->(tc19:TargetCompetency {id : "tc19"})
	SET tc19.label = "Detect theological bias in translations and to help translation teams to remove those biases from the translation."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID17"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp17)
	Merge (cmp17)-[:TARGET_VALUE_IS {planId: "2", min: 3}]->(ms)
	Merge (cmp17)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms2)
	...
	MERGE (grp9)-[:HAS_SHORT_NAME]->(sn29:ShortName {id : "sn29"})
	SET sn29.label = "B) Work with translation team"

	//Comp		Trans-GUID18 in Trans-Proc-Teams
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 30, planId: "2"}]->(cmp18:Competency {id: "Trans-GUID18"})-[:IS_IN_GROUP {order: 30, planId: "2"}]->(grp9)
	SET cmp18.label = "Translation roles",
		cmp18.default_weight = 1.0
	MERGE (cmp18)-[:HAS_SHORT_NAME]->(sn30:ShortName {id : "sn30"})
	SET sn30.label = "Translation roles"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp18)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp18)-[:HAS_TARGET_COMPETENCY]->(tc20:TargetCompetency {id : "tc20"})
	SET tc20.label = "Explain the different roles and responsibilities of translators, translation facilitators, translation consultants. "
	...
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 31, planId: "2"}]->(cmp19:Competency {id: "Trans-GUID19"})-[:IS_IN_GROUP {order: 31, planId: "2"}]->(grp9)
	SET cmp19.label = "Prepare for consultant check",
		cmp19.default_weight = 1.0
	MERGE (cmp19)-[:HAS_SHORT_NAME]->(sn31:ShortName {id : "sn31"})
	SET sn31.label = "Prepare for consultant check"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp19)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp19)-[:HAS_TARGET_COMPETENCY]->(tc21:TargetCompetency {id : "tc21"})
	SET tc21.label = "Prepare for a consultant check, analyze written back translations, write clear and concise exegetical notes for translation teams, to actually do a face-to-face consultant check as well as remote check, and to follow up on the consultant check."
	...
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 32, planId: "2"}]->(cmp20:Competency {id: "Trans-GUID20"})-[:IS_IN_GROUP {order: 32, planId: "2"}]->(grp9)
	SET cmp20.label = "Train translation team",
		cmp20.default_weight = 1.0
	MERGE (cmp20)-[:HAS_SHORT_NAME]->(sn32:ShortName {id : "sn32"})
	SET sn32.label = "Train translation team"
	...
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 33, planId: "2"}]->(cmp21:Competency {id: "Trans-GUID21"})-[:IS_IN_GROUP {order: 33, planId: "2"}]->(grp9)
	SET cmp21.label = "Translation styles",
		cmp21.default_weight = 1.0
	MERGE (cmp21)-[:HAS_SHORT_NAME]->(sn33:ShortName {id : "sn33"})
	SET sn33.label = "Translation styles"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp21)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp21)-[:HAS_TARGET_COMPETENCY]->(tc23:TargetCompetency {id : "tc23"})
	SET tc23.label = "Demonstrate a good understanding of different translation styles appropriate for different audiences and of the sociolinguistic factors that play a role in regard to making decisions related to translation style."
	...
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 34, planId: "2"}]->(cmp22:Competency {id: "Trans-GUID22"})-[:IS_IN_GROUP {order: 34, planId: "2"}]->(grp9)
	SET cmp22.label = "Biblical genres",
		cmp22.default_weight = 1.0
	MERGE (cmp22)-[:HAS_SHORT_NAME]->(sn34:ShortName {id : "sn34"})
	SET sn34.label = "Biblical genres"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp22)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp22)-[:HAS_TARGET_COMPETENCY]->(tc24:TargetCompetency {id : "tc24"})
	SET tc24.label = "Explain the differences to the translation team between different genres in the Bible (narrative, poetic, epistolary, prophetic) and the implications for translation."
	...
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 35, planId: "2"}]->(cmp23:Competency {id: "Trans-GUID23"})-[:IS_IN_GROUP {order: 35, planId: "2"}]->(grp9)
	SET cmp23.label = "Back translations",
		cmp23.default_weight = 1.0
	MERGE (cmp23)-[:HAS_SHORT_NAME]->(sn35:ShortName {id : "sn35"})
	SET sn35.label = "Back translations"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp23)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp23)-[:HAS_TARGET_COMPETENCY]->(tc25:TargetCompetency {id : "tc25"})
	SET tc25.label = "Train the translation team in how to elicit or produce back translations and then verify them."
	...
	//Comp		Trans-GUID24 in Trans-Proc-Teams
	MERGE (grp9)-[:GROUP_HAS_COMPETENCIES_OF {order: 36, planId: "2"}]->(cmp24:Competency {id: "Trans-GUID24"})-[:IS_IN_GROUP {order: 36, planId: "2"}]->(grp9)
	SET cmp24.label = "Comprehension check",
		cmp24.default_weight = 1.0
	MERGE (cmp24)-[:HAS_SHORT_NAME]->(sn36:ShortName {id : "sn36"})
	SET sn36.label = "Comprehension check"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp24)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp24)-[:HAS_TARGET_COMPETENCY]->(tc26:TargetCompetency {id : "tc26"})
	SET tc26.label = "Model, mentor and conduct good comprehension checks (including issues of naturalness and acceptability) with native speakers of the receptor language. "
	...
	MERGE (grp10)-[:HAS_SHORT_NAME]->(sn37:ShortName {id : "sn37"})
	SET sn37.label = "C) Extra textual considerations"

	//Comp		Trans-GUID25 in Trans-Proc-xText
	MERGE (grp10)-[:GROUP_HAS_COMPETENCIES_OF {order: 38, planId: "2"}]->(cmp25:Competency {id: "Trans-GUID25"})-[:IS_IN_GROUP {order: 38, planId: "2"}]->(grp10)
	SET cmp25.label = "Impact of performance on meaning",
		cmp25.default_weight = 1.0
	MERGE (cmp25)-[:HAS_SHORT_NAME]->(sn38:ShortName {id : "sn38"})
	SET sn38.label = "Impact of performance on meaning"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp25)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp25)-[:HAS_TARGET_COMPETENCY]->(tc27:TargetCompetency {id : "tc27"})
	SET tc27.label = "Demonstrate an understanding of the impact of performance upon the meaning of the text/audio. Demonstrate familiarity with different theories of communication, the role of the communicator, the role of the audience, and the impact upon meaning. "
	...
	MERGE (grp10)-[:GROUP_HAS_COMPETENCIES_OF {order: 39, planId: "2"}]->(cmp26:Competency {id: "Trans-GUID26"})-[:IS_IN_GROUP {order: 39, planId: "2"}]->(grp10)
	SET cmp26.label = "Adaptation to audio",
		cmp26.default_weight = 1.0
	MERGE (cmp26)-[:HAS_SHORT_NAME]->(sn39:ShortName {id : "sn39"})
	SET sn39.label = "Adaptation to audio"
...
`);
	...
	MERGE (grp11)-[:HAS_SHORT_NAME]->(sn42:ShortName {id : "sn42"})
	SET sn42.label = "A) Communicate using LWC"

	//Comp		Trans-GUID28 in Trans-Lang-Commun
	MERGE (grp11)-[:GROUP_HAS_COMPETENCIES_OF {order: 43, planId: "2"}]->(cmp28:Competency {id: "Trans-GUID28"})-[:IS_IN_GROUP {order: 43, planId: "2"}]->(grp11)
	SET cmp28.label = "Communicate in LWC",
		cmp28.default_weight = 1.0
	MERGE (cmp28)-[:HAS_SHORT_NAME]->(sn43:ShortName {id : "sn43"})
	SET sn43.label = "Communicate in LWC"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp28)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp28)-[:HAS_TARGET_COMPETENCY]->(tc31:TargetCompetency {id : "tc31"})
	SET tc31.label = "Listen and communicate well in the LWC with people from a variety of language communities."
	...
	MERGE (grp11)-[:GROUP_HAS_COMPETENCIES_OF {order: 44, planId: "2"}]->(cmp29:Competency {id: "Trans-GUID29"})-[:IS_IN_GROUP {order: 44, planId: "2"}]->(grp11)
	SET cmp29.label = "Choose LWC",
		cmp29.default_weight = 1.0
	MERGE (cmp29)-[:HAS_SHORT_NAME]->(sn44:ShortName {id : "sn44"})
	SET sn44.label = "Choose LWC"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp29)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp29)-[:HAS_TARGET_COMPETENCY]->(tc32:TargetCompetency {id : "tc32"})
	SET tc32.label = "Choose the most appropriate language(s) from their repertoire for each context and be aware of the social implications of their language choice."
	...
	MERGE (grp11)-[:GROUP_HAS_COMPETENCIES_OF {order: 45, planId: "2"}]->(cmp30:Competency {id: "Trans-GUID30"})-[:IS_IN_GROUP {order: 45, planId: "2"}]->(grp11)
	SET cmp30.label = "Instruct in LWC",
		cmp30.default_weight = 1.0
	MERGE (cmp30)-[:HAS_SHORT_NAME]->(sn45:ShortName {id : "sn45"})
	SET sn45.label = "Instruct in LWC"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp30)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp30)-[:HAS_TARGET_COMPETENCY]->(tc33:TargetCompetency {id : "tc33"})
	SET tc33.label = "Use the LWC to design and lead instruction in multicultural contexts."
	...
	MERGE (grp11)-[:GROUP_HAS_COMPETENCIES_OF {order: 46, planId: "2"}]->(cmp31:Competency {id: "Trans-GUID31"})-[:IS_IN_GROUP {order: 46, planId: "2"}]->(grp11)
	SET cmp31.label = "Mentor in LWC",
		cmp31.default_weight = 1.0
	MERGE (cmp31)-[:HAS_SHORT_NAME]->(sn46:ShortName {id : "sn46"})
	SET sn46.label = "Mentor in LWC"
	...
	MERGE (grp11)-[:GROUP_HAS_COMPETENCIES_OF {order: 47, planId: "2"}]->(cmp32:Competency {id: "Trans-GUID32"})-[:IS_IN_GROUP {order: 47, planId: "2"}]->(grp11)
	SET cmp32.label = "Present in LWC",
		cmp32.default_weight = 1.0
	MERGE (cmp32)-[:HAS_SHORT_NAME]->(sn47:ShortName {id : "sn47"})
	SET sn47.label = "Present in LWC"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp32)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp32)-[:HAS_TARGET_COMPETENCY]->(tc35:TargetCompetency {id : "tc35"})
	SET tc35.label = "Prepare proposals, reports, and presentations in the LWC."
	...
	//Comp		Trans-GUID33 in Trans-Lang-Commun
	MERGE (grp11)-[:GROUP_HAS_COMPETENCIES_OF {order: 48, planId: "2"}]->(cmp33:Competency {id: "Trans-GUID33"})-[:IS_IN_GROUP {order: 48, planId: "2"}]->(grp11)
	SET cmp33.label = "Assess in LWC",
		cmp33.default_weight = 1.0
	MERGE (cmp33)-[:HAS_SHORT_NAME]->(sn48:ShortName {id : "sn48"})
	SET sn48.label = "Assess in LWC"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp33)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp33)-[:HAS_TARGET_COMPETENCY]->(tc36:TargetCompetency {id : "tc36"})
	SET tc36.label = "Assess and evaluate fairly the work of individuals and project teams in the LWC."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID33"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp33)
	Merge (cmp33)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms)
	...
	MERGE (grp12)-[:HAS_SHORT_NAME]->(sn49:ShortName {id : "sn49"})
	SET sn49.label = "B) Creative use of LWC"

	//Comp		Trans-GUID34 in Trans-Lang-Creative
	MERGE (grp12)-[:GROUP_HAS_COMPETENCIES_OF {order: 50, planId: "2"}]->(cmp34:Competency {id: "Trans-GUID34"})-[:IS_IN_GROUP {order: 50, planId: "2"}]->(grp12)
	SET cmp34.label = "Create materials in LWC",
		cmp34.default_weight = 1.0
	MERGE (cmp34)-[:HAS_SHORT_NAME]->(sn50:ShortName {id : "sn50"})
	SET sn50.label = "Create materials in LWC"
	...
	MERGE (grp12)-[:GROUP_HAS_COMPETENCIES_OF {order: 51, planId: "2"}]->(cmp35:Competency {id: "Trans-GUID35"})-[:IS_IN_GROUP {order: 51, planId: "2"}]->(grp12)
	SET cmp35.label = "Participate in community in LWC",
		cmp35.default_weight = 1.0
	MERGE (cmp35)-[:HAS_SHORT_NAME]->(sn51:ShortName {id : "sn51"})
	SET sn51.label = "Participate in community in LWC"
	...
	MERGE (grp12)-[:GROUP_HAS_COMPETENCIES_OF {order: 52, planId: "2"}]->(cmp36:Competency {id: "Trans-GUID36"})-[:IS_IN_GROUP {order: 52, planId: "2"}]->(grp12)
	SET cmp36.label = "Research in LWC",
		cmp36.default_weight = 1.0
	MERGE (cmp36)-[:HAS_SHORT_NAME]->(sn52:ShortName {id : "sn52"})
	SET sn52.label = "Research in LWC"
	...
	//Comp		Trans-GUID37 in Trans-Lang-Creative
	MERGE (grp12)-[:GROUP_HAS_COMPETENCIES_OF {order: 53, planId: "2"}]->(cmp37:Competency {id: "Trans-GUID37"})-[:IS_IN_GROUP {order: 53, planId: "2"}]->(grp12)
	SET cmp37.label = "Write in LWC for L2 speakers",
		cmp37.default_weight = 1.0
	MERGE (cmp37)-[:HAS_SHORT_NAME]->(sn53:ShortName {id : "sn53"})
	SET sn53.label = "Write in LWC for L2 speakers"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp37)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp37)-[:HAS_TARGET_COMPETENCY]->(tc40:TargetCompetency {id : "tc40"})
	SET tc40.label = "Write in the LWC so that L2 readers can understand easily."
	...
	MERGE (grp13)-[:HAS_SHORT_NAME]->(sn54:ShortName {id : "sn54"})
	SET sn54.label = "C) Cross-cultural Research"

	//Comp		Trans-GUID38 in Trans-Lang-CrossCulture
	MERGE (grp13)-[:GROUP_HAS_COMPETENCIES_OF {order: 55, planId: "2"}]->(cmp38:Competency {id: "Trans-GUID38"})-[:IS_IN_GROUP {order: 55, planId: "2"}]->(grp13)
	SET cmp38.label = "Cross-cultural research: execute",
		cmp38.default_weight = 1.0
	MERGE (cmp38)-[:HAS_SHORT_NAME]->(sn55:ShortName {id : "sn55"})
	SET sn55.label = "Cross-cultural research: execute"
	...
	MERGE (grp13)-[:GROUP_HAS_COMPETENCIES_OF {order: 56, planId: "2"}]->(cmp39:Competency {id: "Trans-GUID39"})-[:IS_IN_GROUP {order: 56, planId: "2"}]->(grp13)
	SET cmp39.label = "Cross-cultural research: analyze",
		cmp39.default_weight = 1.0
	MERGE (cmp39)-[:HAS_SHORT_NAME]->(sn56:ShortName {id : "sn56"})
	SET sn56.label = "Cross-cultural research: analyze"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp39)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp39)-[:HAS_TARGET_COMPETENCY]->(tc42:TargetCompetency {id : "tc42"})
	SET tc42.label = "Find implications of results and share them with others."
	...
	MERGE (grp13)-[:GROUP_HAS_COMPETENCIES_OF {order: 57, planId: "2"}]->(cmp40:Competency {id: "Trans-GUID40"})-[:IS_IN_GROUP {order: 57, planId: "2"}]->(grp13)
	SET cmp40.label = "Cross-cultural research: apply",
		cmp40.default_weight = 1.0
	MERGE (cmp40)-[:HAS_SHORT_NAME]->(sn57:ShortName {id : "sn57"})
	SET sn57.label = "Cross-cultural research: apply"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp40)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp40)-[:HAS_TARGET_COMPETENCY]->(tc43:TargetCompetency {id : "tc43"})
	SET tc43.label = "Use research findings to improve consulting strategies and practices."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID40"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp40)
	Merge (cmp40)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms)
	Merge (cmp40)-[:TARGET_VALUE_IS {planId: "2", min: 1}]->(ms2)

	//Comp		Trans-GUID41 in Trans-Lang-CrossCulture
	MERGE (grp13)-[:GROUP_HAS_COMPETENCIES_OF {order: 58, planId: "2"}]->(cmp41:Competency {id: "Trans-GUID41"})-[:IS_IN_GROUP {order: 58, planId: "2"}]->(grp13)
	SET cmp41.label = "Cross-cultural research: document",
		cmp41.default_weight = 1.0
	MERGE (cmp41)-[:HAS_SHORT_NAME]->(sn58:ShortName {id : "sn58"})
	SET sn58.label = "Cross-cultural research: document"
	...
	//Comp		Trans-GUID42 in Trans-Lang-CrossCulture
	MERGE (grp13)-[:GROUP_HAS_COMPETENCIES_OF {order: 59, planId: "2"}]->(cmp42:Competency {id: "Trans-GUID42"})-[:IS_IN_GROUP {order: 59, planId: "2"}]->(grp13)
	SET cmp42.label = "Cross-cultural research: train",
		cmp42.default_weight = 1.0
	MERGE (cmp42)-[:HAS_SHORT_NAME]->(sn59:ShortName {id : "sn59"})
	SET sn59.label = "Cross-cultural research: train"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp42)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp42)-[:HAS_TARGET_COMPETENCY]->(tc45:TargetCompetency {id : "tc45"})
	SET tc45.label = "Help others to be reflexive and to apply basic anthropological skills."
...
`);
	...
	SET cat5.label = "Working in a Multicultural Environment",
		cat5.color = "violet"
	MERGE (cat5)-[:HAS_SHORT_NAME]->(sn60:ShortName {id : "sn60"})
	SET sn60.label = "Working in a Multicultural Environment"
	MERGE (cat5)-[:HAS_TARGET_COMPETENCY]->(tc46:TargetCompetency {id : "tc46"})
	SET tc46.label = "Consulting effectively in a multicultural environment involves understanding the cultural values of each person and the dynamics of communication in a group or team situation. It requires being able to adapt one�s own communication style, and to give and receive feedback in a manner that befits the group dynamics."
	...
	MERGE (grp14)-[:HAS_SHORT_NAME]->(sn61:ShortName {id : "sn61"})
	SET sn61.label = "A) Work with others cross-culturally"

	//Comp		Trans-GUID43 in Trans-Culture-Work
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 62, planId: "2"}]->(cmp43:Competency {id: "Trans-GUID43"})-[:IS_IN_GROUP {order: 62, planId: "2"}]->(grp14)
	SET cmp43.label = "Understand patterns of communication",
		cmp43.default_weight = 1.0
	MERGE (cmp43)-[:HAS_SHORT_NAME]->(sn62:ShortName {id : "sn62"})
	SET sn62.label = "Understand patterns of communication"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp43)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp43)-[:HAS_TARGET_COMPETENCY]->(tc47:TargetCompetency {id : "tc47"})
	SET tc47.label = "Identify and understand different patterns of communication."
	...
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 63, planId: "2"}]->(cmp44:Competency {id: "Trans-GUID44"})-[:IS_IN_GROUP {order: 63, planId: "2"}]->(grp14)
	SET cmp44.label = "Relate in different communication styles",
		cmp44.default_weight = 1.0
	MERGE (cmp44)-[:HAS_SHORT_NAME]->(sn63:ShortName {id : "sn63"})
	SET sn63.label = "Relate in different communication styles"
	...
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 64, planId: "2"}]->(cmp45:Competency {id: "Trans-GUID45"})-[:IS_IN_GROUP {order: 64, planId: "2"}]->(grp14)
	SET cmp45.label = "Complete team assignments",
		cmp45.default_weight = 1.0
	MERGE (cmp45)-[:HAS_SHORT_NAME]->(sn64:ShortName {id : "sn64"})
	SET sn64.label = "Complete team assignments"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp45)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp45)-[:HAS_TARGET_COMPETENCY]->(tc49:TargetCompetency {id : "tc49"})
	SET tc49.label = "Complete team assignments acceptably where applicable."
	...
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 65, planId: "2"}]->(cmp46:Competency {id: "Trans-GUID46"})-[:IS_IN_GROUP {order: 65, planId: "2"}]->(grp14)
	SET cmp46.label = "Account for cultural backgrounds",
		cmp46.default_weight = 1.0
	MERGE (cmp46)-[:HAS_SHORT_NAME]->(sn65:ShortName {id : "sn65"})
	SET sn65.label = "Account for cultural backgrounds"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp46)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp46)-[:HAS_TARGET_COMPETENCY]->(tc50:TargetCompetency {id : "tc50"})
	SET tc50.label = "Account for people�s different cultural backgrounds and facilitate positive group dynamics and patterns of working that allow people to contribute more effectively."
	...
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 66, planId: "2"}]->(cmp47:Competency {id: "Trans-GUID47"})-[:IS_IN_GROUP {order: 66, planId: "2"}]->(grp14)
	SET cmp47.label = "Adapt to leadership styles",
		cmp47.default_weight = 1.0
	MERGE (cmp47)-[:HAS_SHORT_NAME]->(sn66:ShortName {id : "sn66"})
	SET sn66.label = "Adapt to leadership styles"
	...
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 67, planId: "2"}]->(cmp48:Competency {id: "Trans-GUID48"})-[:IS_IN_GROUP {order: 67, planId: "2"}]->(grp14)
	SET cmp48.label = "Participate in decision-making",
		cmp48.default_weight = 1.0
	MERGE (cmp48)-[:HAS_SHORT_NAME]->(sn67:ShortName {id : "sn67"})
	SET sn67.label = "Participate in decision-making"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp48)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp48)-[:HAS_TARGET_COMPETENCY]->(tc52:TargetCompetency {id : "tc52"})
	SET tc52.label = "Participate effectively in decision-making processes."
	...
	//Comp		Trans-GUID49 in Trans-Culture-Work
	MERGE (grp14)-[:GROUP_HAS_COMPETENCIES_OF {order: 68, planId: "2"}]->(cmp49:Competency {id: "Trans-GUID49"})-[:IS_IN_GROUP {order: 68, planId: "2"}]->(grp14)
	SET cmp49.label = "Guide and mentor teams appropriately",
		cmp49.default_weight = 1.0
	MERGE (cmp49)-[:HAS_SHORT_NAME]->(sn68:ShortName {id : "sn68"})
	SET sn68.label = "Guide and mentor teams appropriately"
	...
	MERGE (grp15)-[:HAS_SHORT_NAME]->(sn69:ShortName {id : "sn69"})
	SET sn69.label = "B) Take a lead cross-culturally"

	//Comp		Trans-GUID50 in Trans-Culture-Lead
	MERGE (grp15)-[:GROUP_HAS_COMPETENCIES_OF {order: 70, planId: "2"}]->(cmp50:Competency {id: "Trans-GUID50"})-[:IS_IN_GROUP {order: 70, planId: "2"}]->(grp15)
	SET cmp50.label = "Train cross-culturally",
		cmp50.default_weight = 1.0
	MERGE (cmp50)-[:HAS_SHORT_NAME]->(sn70:ShortName {id : "sn70"})
	SET sn70.label = "Train cross-culturally"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp50)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp50)-[:HAS_TARGET_COMPETENCY]->(tc54:TargetCompetency {id : "tc54"})
	SET tc54.label = "Communicate and participate effectively in cross-cultural mentoring, training, and project team activities, guiding translation teams to find translation problems and to solve them rather than solving the problems for them."
	...
	MERGE (grp15)-[:GROUP_HAS_COMPETENCIES_OF {order: 71, planId: "2"}]->(cmp51:Competency {id: "Trans-GUID51"})-[:IS_IN_GROUP {order: 71, planId: "2"}]->(grp15)
	SET cmp51.label = "Mentor peers cross-culturally",
		cmp51.default_weight = 1.0
	MERGE (cmp51)-[:HAS_SHORT_NAME]->(sn71:ShortName {id : "sn71"})
	SET sn71.label = "Mentor peers cross-culturally"
	...
	//Comp		Trans-GUID52 in Trans-Culture-Lead
	MERGE (grp15)-[:GROUP_HAS_COMPETENCIES_OF {order: 72, planId: "2"}]->(cmp52:Competency {id: "Trans-GUID52"})-[:IS_IN_GROUP {order: 72, planId: "2"}]->(grp15)
	SET cmp52.label = "Assess cross-culturally",
		cmp52.default_weight = 1.0
	MERGE (cmp52)-[:HAS_SHORT_NAME]->(sn72:ShortName {id : "sn72"})
	SET sn72.label = "Assess cross-culturally"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp52)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp52)-[:HAS_TARGET_COMPETENCY]->(tc56:TargetCompetency {id : "tc56"})
	SET tc56.label = "Assess and evaluate the work of individuals in a multicultural environment, and communicate that assessment in a culturally appropriate way: knowing when to be flexible and when to insist that a certain interpretation or rendering is not acceptable; clearly explaining why a certain interpretation or rendering is not acceptable."
...
`);
	...
	SET cat6.label = "Mentoring",
		cat6.color = "brown"
	MERGE (cat6)-[:HAS_SHORT_NAME]->(sn73:ShortName {id : "sn73"})
	SET sn73.label = "Mentoring"
	MERGE (cat6)-[:HAS_TARGET_COMPETENCY]->(tc57:TargetCompetency {id : "tc57"})
	SET tc57.label = "Mentoring occurs informally when an individual is accompanied for a while by a friend or colleague more experienced in the field who offers guidance and advice. Mentoring formally is an organized and structured process with defined roles and responsibilities for guiding people in their professional development. There is an agreement that establishes the goals of the mentoring, means of evaluation and report of progress, and how the mentoring will finish. Formal mentoring seeks to ensure that everyone in an organization who needs mentoring receives it. (Some of these competencies overlap with interpersonal skills, but they are listed here because they are especially important for mentors.)"
	...
	MERGE (grp16)-[:HAS_SHORT_NAME]->(sn74:ShortName {id : "sn74"})
	SET sn74.label = "A) Knowledge"

	//Comp		Trans-GUID53 in Trans-Mentor-Knowledge
	MERGE (grp16)-[:GROUP_HAS_COMPETENCIES_OF {order: 75, planId: "2"}]->(cmp53:Competency {id: "Trans-GUID53"})-[:IS_IN_GROUP {order: 75, planId: "2"}]->(grp16)
	SET cmp53.label = "Mentoring and Coaching",
		cmp53.default_weight = 1.0
	MERGE (cmp53)-[:HAS_SHORT_NAME]->(sn75:ShortName {id : "sn75"})
	SET sn75.label = "Mentoring and Coaching"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp53)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp53)-[:HAS_TARGET_COMPETENCY]->(tc58:TargetCompetency {id : "tc58"})
	SET tc58.label = "Describe how formal mentoring is different from informal mentoring and coaching/facilitation."
	...
	MERGE (grp16)-[:GROUP_HAS_COMPETENCIES_OF {order: 76, planId: "2"}]->(cmp54:Competency {id: "Trans-GUID54"})-[:IS_IN_GROUP {order: 76, planId: "2"}]->(grp16)
	SET cmp54.label = "Mentor and Mentoree in Formal Mentoring",
		cmp54.default_weight = 1.0
	MERGE (cmp54)-[:HAS_SHORT_NAME]->(sn76:ShortName {id : "sn76"})
	SET sn76.label = "Mentor and Mentoree in Formal Mentoring"
	...
	//Comp		Trans-GUID55 in Trans-Mentor-Knowledge
	MERGE (grp16)-[:GROUP_HAS_COMPETENCIES_OF {order: 77, planId: "2"}]->(cmp55:Competency {id: "Trans-GUID55"})-[:IS_IN_GROUP {order: 77, planId: "2"}]->(grp16)
	SET cmp55.label = "Administrative expectations",
		cmp55.default_weight = 1.0
	MERGE (cmp55)-[:HAS_SHORT_NAME]->(sn77:ShortName {id : "sn77"})
	SET sn77.label = "Administrative expectations"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp55)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp55)-[:HAS_TARGET_COMPETENCY]->(tc60:TargetCompetency {id : "tc60"})
	SET tc60.label = "Know the administrative tasks expected by the mentoree�s organization."
	...
	MERGE (grp17)-[:HAS_SHORT_NAME]->(sn78:ShortName {id : "sn78"})
	SET sn78.label = "B) Skills"

	//Comp		Trans-GUID56 in Trans-Mentor-Skills
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 79, planId: "2"}]->(cmp56:Competency {id: "Trans-GUID56"})-[:IS_IN_GROUP {order: 79, planId: "2"}]->(grp17)
	SET cmp56.label = "Expectations for relationship",
		cmp56.default_weight = 1.0
	MERGE (cmp56)-[:HAS_SHORT_NAME]->(sn79:ShortName {id : "sn79"})
	SET sn79.label = "Expectations for relationship"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp56)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp56)-[:HAS_TARGET_COMPETENCY]->(tc61:TargetCompetency {id : "tc61"})
	SET tc61.label = "Set and follow protocols and expectations for the relationship (how goals are going to be achieved, how to handle disagreements, how to track progress, etc.)."
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 80, planId: "2"}]->(cmp57:Competency {id: "Trans-GUID57"})-[:IS_IN_GROUP {order: 80, planId: "2"}]->(grp17)
	SET cmp57.label = "Reciprocal relationship",
		cmp57.default_weight = 1.0
	MERGE (cmp57)-[:HAS_SHORT_NAME]->(sn80:ShortName {id : "sn80"})
	SET sn80.label = "Reciprocal relationship"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp57)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp57)-[:HAS_TARGET_COMPETENCY]->(tc62:TargetCompetency {id : "tc62"})
	SET tc62.label = "Develop a reciprocal relationship through sharing experiences, exploring expectations, and clarifying the roles of mentor and mentoree."
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 81, planId: "2"}]->(cmp58:Competency {id: "Trans-GUID58"})-[:IS_IN_GROUP {order: 81, planId: "2"}]->(grp17)
	SET cmp58.label = "Mutual learning",
		cmp58.default_weight = 1.0
	MERGE (cmp58)-[:HAS_SHORT_NAME]->(sn81:ShortName {id : "sn81"})
	SET sn81.label = "Mutual learning"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp58)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp58)-[:HAS_TARGET_COMPETENCY]->(tc63:TargetCompetency {id : "tc63"})
	SET tc63.label = "Encourage a relationship of mutual learning between mentor and mentoree (with an attitude of �Let us learn together�)."
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 82, planId: "2"}]->(cmp59:Competency {id: "Trans-GUID59"})-[:IS_IN_GROUP {order: 82, planId: "2"}]->(grp17)
	SET cmp59.label = "Listen to encourage growth",
		cmp59.default_weight = 1.0
	MERGE (cmp59)-[:HAS_SHORT_NAME]->(sn82:ShortName {id : "sn82"})
	SET sn82.label = "Listen to encourage growth"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp59)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp59)-[:HAS_TARGET_COMPETENCY]->(tc64:TargetCompetency {id : "tc64"})
	SET tc64.label = "Listen actively and use encouragement to facilitate growth."
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 83, planId: "2"}]->(cmp60:Competency {id: "Trans-GUID60"})-[:IS_IN_GROUP {order: 83, planId: "2"}]->(grp17)
	SET cmp60.label = "Goals for continuous improvement",
		cmp60.default_weight = 1.0
	MERGE (cmp60)-[:HAS_SHORT_NAME]->(sn83:ShortName {id : "sn83"})
	SET sn83.label = "Goals for continuous improvement"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp60)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp60)-[:HAS_TARGET_COMPETENCY]->(tc65:TargetCompetency {id : "tc65"})
	SET tc65.label = "Provide guidance in reflecting upon and interpreting experiences and performance in order to develop goals in a plan for continuous improvement."
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 84, planId: "2"}]->(cmp61:Competency {id: "Trans-GUID61"})-[:IS_IN_GROUP {order: 84, planId: "2"}]->(grp17)
	SET cmp61.label = "Recognize goals met",
		cmp61.default_weight = 1.0
	MERGE (cmp61)-[:HAS_SHORT_NAME]->(sn84:ShortName {id : "sn84"})
	SET sn84.label = "Recognize goals met"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp61)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp61)-[:HAS_TARGET_COMPETENCY]->(tc66:TargetCompetency {id : "tc66"})
	SET tc66.label = "Set clear goals for the mentoree to achieve, recognize and articulate when goals are met. "
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 86, planId: "2"}]->(cmp63:Competency {id: "Trans-GUID63"})-[:IS_IN_GROUP {order: 86, planId: "2"}]->(grp17)
	SET cmp63.label = "Provide feedback",
		cmp63.default_weight = 1.0
	MERGE (cmp63)-[:HAS_SHORT_NAME]->(sn86:ShortName {id : "sn86"})
	SET sn86.label = "Provide feedback"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp63)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp63)-[:HAS_TARGET_COMPETENCY]->(tc68:TargetCompetency {id : "tc68"})
	SET tc68.label = "Give and receive sincere feedback about the mentoree's performance, behavior and development with care and compassion in a culturally appropriate way (identifying both positive growth and areas still needing improvement)."
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 87, planId: "2"}]->(cmp64:Competency {id: "Trans-GUID64"})-[:IS_IN_GROUP {order: 87, planId: "2"}]->(grp17)
	SET cmp64.label = "Create safe space",
		cmp64.default_weight = 1.0
	MERGE (cmp64)-[:HAS_SHORT_NAME]->(sn87:ShortName {id : "sn87"})
	SET sn87.label = "Create safe space"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp64)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp64)-[:HAS_TARGET_COMPETENCY]->(tc69:TargetCompetency {id : "tc69"})
	SET tc69.label = "Create a safe space for the mentoree to excel, shine and build confidence. "
	...
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 88, planId: "2"}]->(cmp65:Competency {id: "Trans-GUID65"})-[:IS_IN_GROUP {order: 88, planId: "2"}]->(grp17)
	SET cmp65.label = "Advocate on behalf of mentoree",
		cmp65.default_weight = 1.0
	MERGE (cmp65)-[:HAS_SHORT_NAME]->(sn88:ShortName {id : "sn88"})
	SET sn88.label = "Advocate on behalf of mentoree"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp65)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp65)-[:HAS_TARGET_COMPETENCY]->(tc70:TargetCompetency {id : "tc70"})
	SET tc70.label = "Advocate on behalf of one�s mentorees so they have opportunities to get experiences necessary for their growth."
	...
	//Comp		Trans-GUID66 in Trans-Mentor-Skills
	MERGE (grp17)-[:GROUP_HAS_COMPETENCIES_OF {order: 89, planId: "2"}]->(cmp66:Competency {id: "Trans-GUID66"})-[:IS_IN_GROUP {order: 89, planId: "2"}]->(grp17)
	SET cmp66.label = "Maintain confidentiality",
		cmp66.default_weight = 1.0
	MERGE (cmp66)-[:HAS_SHORT_NAME]->(sn89:ShortName {id : "sn89"})
	SET sn89.label = "Maintain confidentiality"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp66)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp66)-[:HAS_TARGET_COMPETENCY]->(tc71:TargetCompetency {id : "tc71"})
	SET tc71.label = "Maintain confidentiality when sharing experiences, discussing issues and solving problems."
	...
	MERGE (grp18)-[:HAS_SHORT_NAME]->(sn90:ShortName {id : "sn90"})
	SET sn90.label = "C) Attitudes"

	//Comp		Trans-GUID67 in Trans-Mentor-Attitudes
	MERGE (grp18)-[:GROUP_HAS_COMPETENCIES_OF {order: 91, planId: "2"}]->(cmp67:Competency {id: "Trans-GUID67"})-[:IS_IN_GROUP {order: 91, planId: "2"}]->(grp18)
	SET cmp67.label = "Respect",
		cmp67.default_weight = 1.0
	MERGE (cmp67)-[:HAS_SHORT_NAME]->(sn91:ShortName {id : "sn91"})
	SET sn91.label = "Respect"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp67)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp67)-[:HAS_TARGET_COMPETENCY]->(tc72:TargetCompetency {id : "tc72"})
	SET tc72.label = "Respects and values other people�s opinions, knowledge, and experience."
	...
	MERGE (grp18)-[:GROUP_HAS_COMPETENCIES_OF {order: 92, planId: "2"}]->(cmp68:Competency {id: "Trans-GUID68"})-[:IS_IN_GROUP {order: 92, planId: "2"}]->(grp18)
	SET cmp68.label = "Humility",
		cmp68.default_weight = 1.0
	MERGE (cmp68)-[:HAS_SHORT_NAME]->(sn92:ShortName {id : "sn92"})
	SET sn92.label = "Humility"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp68)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp68)-[:HAS_TARGET_COMPETENCY]->(tc73:TargetCompetency {id : "tc73"})
	SET tc73.label = "Is realistically self-aware and humble."
	...
	MERGE (grp18)-[:GROUP_HAS_COMPETENCIES_OF {order: 93, planId: "2"}]->(cmp69:Competency {id: "Trans-GUID69"})-[:IS_IN_GROUP {order: 93, planId: "2"}]->(grp18)
	SET cmp69.label = "Rejoice with others",
		cmp69.default_weight = 1.0
	MERGE (cmp69)-[:HAS_SHORT_NAME]->(sn93:ShortName {id : "sn93"})
	SET sn93.label = "Rejoice with others"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp69)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp69)-[:HAS_TARGET_COMPETENCY]->(tc74:TargetCompetency {id : "tc74"})
	SET tc74.label = "Delights in other people�s success."
	...
	MERGE (grp18)-[:GROUP_HAS_COMPETENCIES_OF {order: 94, planId: "2"}]->(cmp70:Competency {id: "Trans-GUID70"})-[:IS_IN_GROUP {order: 94, planId: "2"}]->(grp18)
	SET cmp70.label = "Open",
	...
	SET tc75.label = "Is willing to be open about one�s own successes and failures (wise and appropriate self-disclosure)."
	...
	MERGE (grp18)-[:GROUP_HAS_COMPETENCIES_OF {order: 95, planId: "2"}]->(cmp71:Competency {id: "Trans-GUID71"})-[:IS_IN_GROUP {order: 95, planId: "2"}]->(grp18)
	SET cmp71.label = "Self-aware",
		cmp71.default_weight = 1.0
	MERGE (cmp71)-[:HAS_SHORT_NAME]->(sn95:ShortName {id : "sn95"})
	SET sn95.label = "Self-aware"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp71)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp71)-[:HAS_TARGET_COMPETENCY]->(tc76:TargetCompetency {id : "tc76"})
	SET tc76.label = "Understands the limitations of one�s own experiences and perspective, and minimize any negative impact on the mentoree."
	...
	//Comp		Trans-GUID72 in Trans-Mentor-Attitudes
	MERGE (grp18)-[:GROUP_HAS_COMPETENCIES_OF {order: 96, planId: "2"}]->(cmp72:Competency {id: "Trans-GUID72"})-[:IS_IN_GROUP {order: 96, planId: "2"}]->(grp18)
	SET cmp72.label = "Expect mentoree to mentor others",
		cmp72.default_weight = 1.0
	MERGE (cmp72)-[:HAS_SHORT_NAME]->(sn96:ShortName {id : "sn96"})
	SET sn96.label = "Expect mentoree to mentor others"
...
`);
	...
	MERGE (grp19)-[:HAS_SHORT_NAME]->(sn98:ShortName {id : "sn98"})
	SET sn98.label = "A) General"

	//Comp		Trans-GUID73 in Trans-Cons-Gen
	MERGE (grp19)-[:GROUP_HAS_COMPETENCIES_OF {order: 99, planId: "2"}]->(cmp73:Competency {id: "Trans-GUID73"})-[:IS_IN_GROUP {order: 99, planId: "2"}]->(grp19)
	SET cmp73.label = "Establish collaborative relationships",
		cmp73.default_weight = 1.0
	MERGE (cmp73)-[:HAS_SHORT_NAME]->(sn99:ShortName {id : "sn99"})
	SET sn99.label = "Establish collaborative relationships"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp73)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp73)-[:HAS_TARGET_COMPETENCY]->(tc79:TargetCompetency {id : "tc79"})
	SET tc79.label = "Establish collaborative relationships with individual clients and within an organization."
	...
	MERGE (grp19)-[:GROUP_HAS_COMPETENCIES_OF {order: 100, planId: "2"}]->(cmp74:Competency {id: "Trans-GUID74"})-[:IS_IN_GROUP {order: 100, planId: "2"}]->(grp19)
	SET cmp74.label = "Represent organization with diplomacy",
		cmp74.default_weight = 1.0
	MERGE (cmp74)-[:HAS_SHORT_NAME]->(sn100:ShortName {id : "sn100"})
	SET sn100.label = "Represent organization with diplomacy"
	...
	//Comp		Trans-GUID75 in Trans-Cons-Gen
	MERGE (grp19)-[:GROUP_HAS_COMPETENCIES_OF {order: 101, planId: "2"}]->(cmp75:Competency {id: "Trans-GUID75"})-[:IS_IN_GROUP {order: 101, planId: "2"}]->(grp19)
	SET cmp75.label = " Articulate different stages of the consulting process",
		cmp75.default_weight = 1.0
	MERGE (cmp75)-[:HAS_SHORT_NAME]->(sn101:ShortName {id : "sn101"})
	SET sn101.label = " Articulate different stages of the consulting process"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp75)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp75)-[:HAS_TARGET_COMPETENCY]->(tc81:TargetCompetency {id : "tc81"})
	SET tc81.label = "Articulate an understanding of different stages/facets of the consulting process, including contracting (defining the assignment and clarifying roles), discovery, feedback, engagement/implementation, and evaluation."
	...
	MERGE (grp20)-[:HAS_SHORT_NAME]->(sn102:ShortName {id : "sn102"})
	SET sn102.label = "B) Contracting"

	//Comp		Trans-GUID76 in Trans-Cons-Cont
	MERGE (grp20)-[:GROUP_HAS_COMPETENCIES_OF {order: 103, planId: "2"}]->(cmp76:Competency {id: "Trans-GUID76"})-[:IS_IN_GROUP {order: 103, planId: "2"}]->(grp20)
	SET cmp76.label = "Define expectations",
		cmp76.default_weight = 1.0
	MERGE (cmp76)-[:HAS_SHORT_NAME]->(sn103:ShortName {id : "sn103"})
	SET sn103.label = "Define expectations"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp76)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp76)-[:HAS_TARGET_COMPETENCY]->(tc82:TargetCompetency {id : "tc82"})
	SET tc82.label = "Define what is expected of the consultation process, what the boundaries are, what support the consultant requires, and what and when the consultant and consultee are expected to deliver."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID76"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp76)
	Merge (cmp76)-[:TARGET_VALUE_IS {planId: "2", min: 3}]->(ms)
	Merge (cmp76)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms2)

	//Comp		Trans-GUID77 in Trans-Cons-Cont
	MERGE (grp20)-[:GROUP_HAS_COMPETENCIES_OF {order: 104, planId: "2"}]->(cmp77:Competency {id: "Trans-GUID77"})-[:IS_IN_GROUP {order: 104, planId: "2"}]->(grp20)
	SET cmp77.label = "Listen to needs of team",
		cmp77.default_weight = 1.0
	MERGE (cmp77)-[:HAS_SHORT_NAME]->(sn104:ShortName {id : "sn104"})
	SET sn104.label = "Listen to needs of team"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp77)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp77)-[:HAS_TARGET_COMPETENCY]->(tc83:TargetCompetency {id : "tc83"})
	SET tc83.label = "Listen to and understand the needs that the client/team/organization expresses. Understands the interplay between budget, time, scope, project goals, and their impact on quality and can successfully shape and navigate these things to meet desired parameters."
	...
	MERGE (grp21)-[:HAS_SHORT_NAME]->(sn105:ShortName {id : "sn105"})
	SET sn105.label = "C) Discovery and feedback"

	//Comp		Trans-GUID78 in Trans-Cons-Disc
	MERGE (grp21)-[:GROUP_HAS_COMPETENCIES_OF {order: 106, planId: "2"}]->(cmp78:Competency {id: "Trans-GUID78"})-[:IS_IN_GROUP {order: 106, planId: "2"}]->(grp21)
	SET cmp78.label = "Guide team through consultant process",
		cmp78.default_weight = 1.0
	MERGE (cmp78)-[:HAS_SHORT_NAME]->(sn106:ShortName {id : "sn106"})
	SET sn106.label = "Guide team through consultant process"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp78)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp78)-[:HAS_TARGET_COMPETENCY]->(tc84:TargetCompetency {id : "tc84"})
	SET tc84.label = "Organize a process of discovery, data collection, and dialogue that will help analyze the expressed needs/problems. (Is able to guide the team through a consulting session, including preparation and follow-up)."
	...
	MERGE (grp21)-[:GROUP_HAS_COMPETENCIES_OF {order: 107, planId: "2"}]->(cmp79:Competency {id: "Trans-GUID79"})-[:IS_IN_GROUP {order: 107, planId: "2"}]->(grp21)
	SET cmp79.label = "Provide feedback",
		cmp79.default_weight = 1.0
	MERGE (cmp79)-[:HAS_SHORT_NAME]->(sn107:ShortName {id : "sn107"})
	SET sn107.label = "Provide feedback"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp79)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp79)-[:HAS_TARGET_COMPETENCY]->(tc85:TargetCompetency {id : "tc85"})
	SET tc85.label = "Provide feedback and recommendations from the discovery process in a way that maximizes client commitment and engagement (write a consultant report)."
	...
	//Comp		Trans-GUID80 in Trans-Cons-Disc
	MERGE (grp21)-[:GROUP_HAS_COMPETENCIES_OF {order: 108, planId: "2"}]->(cmp80:Competency {id: "Trans-GUID80"})-[:IS_IN_GROUP {order: 108, planId: "2"}]->(grp21)
	SET cmp80.label = "Deal with resistance to change",
		cmp80.default_weight = 1.0
	MERGE (cmp80)-[:HAS_SHORT_NAME]->(sn108:ShortName {id : "sn108"})
	SET sn108.label = "Deal with resistance to change"
	...
	MERGE (grp22)-[:HAS_SHORT_NAME]->(sn109:ShortName {id : "sn109"})
	SET sn109.label = "D) Implementation and evaluation"

	//Comp		Trans-GUID81 in Trans-Cons-Eval
	MERGE (grp22)-[:GROUP_HAS_COMPETENCIES_OF {order: 110, planId: "2"}]->(cmp81:Competency {id: "Trans-GUID81"})-[:IS_IN_GROUP {order: 110, planId: "2"}]->(grp22)
	SET cmp81.label = "Define specific assignments",
		cmp81.default_weight = 1.0
	MERGE (cmp81)-[:HAS_SHORT_NAME]->(sn110:ShortName {id : "sn110"})
	SET sn110.label = "Define specific assignments"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp81)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp81)-[:HAS_TARGET_COMPETENCY]->(tc87:TargetCompetency {id : "tc87"})
	SET tc87.label = "Manage meetings which will lead to a plan of action, clearly defining specific assignments needed to implement change."
	...
	MERGE (grp22)-[:GROUP_HAS_COMPETENCIES_OF {order: 111, planId: "2"}]->(cmp82:Competency {id: "Trans-GUID82"})-[:IS_IN_GROUP {order: 111, planId: "2"}]->(grp22)
	SET cmp82.label = "Monitor progress",
		cmp82.default_weight = 1.0
	MERGE (cmp82)-[:HAS_SHORT_NAME]->(sn111:ShortName {id : "sn111"})
	SET sn111.label = "Monitor progress"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp82)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp82)-[:HAS_TARGET_COMPETENCY]->(tc88:TargetCompetency {id : "tc88"})
	SET tc88.label = "Monitor progress during the implementation."
	...
	MERGE (grp22)-[:GROUP_HAS_COMPETENCIES_OF {order: 112, planId: "2"}]->(cmp83:Competency {id: "Trans-GUID83"})-[:IS_IN_GROUP {order: 112, planId: "2"}]->(grp22)
	SET cmp83.label = "Equip the team",
		cmp83.default_weight = 1.0
	MERGE (cmp83)-[:HAS_SHORT_NAME]->(sn112:ShortName {id : "sn112"})
	SET sn112.label = "Equip the team"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp83)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp83)-[:HAS_TARGET_COMPETENCY]->(tc89:TargetCompetency {id : "tc89"})
	SET tc89.label = "Work towards a situation where the organization/client is equipped to carry forward the project independent of the consultant."
	...
	//Comp		Trans-GUID84 in Trans-Cons-Eval
	MERGE (grp22)-[:GROUP_HAS_COMPETENCIES_OF {order: 113, planId: "2"}]->(cmp84:Competency {id: "Trans-GUID84"})-[:IS_IN_GROUP {order: 113, planId: "2"}]->(grp22)
	SET cmp84.label = "Final evaluation",
		cmp84.default_weight = 1.0
	MERGE (cmp84)-[:HAS_SHORT_NAME]->(sn113:ShortName {id : "sn113"})
	SET sn113.label = "Final evaluation"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp84)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp84)-[:HAS_TARGET_COMPETENCY]->(tc90:TargetCompetency {id : "tc90"})
	SET tc90.label = "Provide an evaluation of the process/results before withdrawal (including oral feedback and written reports, as appropriate)."
...
`);
	...
	SET cat8.label = "Technology for Consulting",
		cat8.color = "yellow"
	MERGE (cat8)-[:HAS_SHORT_NAME]->(sn114:ShortName {id : "sn114"})
	SET sn114.label = "Technology for Consulting"
	MERGE (cat8)-[:HAS_TARGET_COMPETENCY]->(tc91:TargetCompetency {id : "tc91"})
	SET tc91.label = "The widespread and growing use of technology means that consultants need to be able to perform from memory the basic, routine, and typical Technology for Consulting tasks, using current Best Practice techniques and tools."
	...
	MERGE (grp23)-[:HAS_SHORT_NAME]->(sn115:ShortName {id : "sn115"})
	SET sn115.label = "A) Translation software"

	//Comp		Trans-GUID85 in Trans-Tech-Soft
	MERGE (grp23)-[:GROUP_HAS_COMPETENCIES_OF {order: 116, planId: "2"}]->(cmp85:Competency {id: "Trans-GUID85"})-[:IS_IN_GROUP {order: 116, planId: "2"}]->(grp23)
	SET cmp85.label = "Use relevant translation software",
		cmp85.default_weight = 1.0
	MERGE (cmp85)-[:HAS_SHORT_NAME]->(sn116:ShortName {id : "sn116"})
	SET sn116.label = "Use relevant translation software"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp85)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp85)-[:HAS_TARGET_COMPETENCY]->(tc92:TargetCompetency {id : "tc92"})
	SET tc92.label = "Demonstrate the ability to use relevant software (e.g. Paratext, Logos, Render, OneStory Editor, HearThis, Transcriber) well (research an exegetical issue, prepare exegetical notes, resolve notes, perform various kinds of checks)."
	...
	//Comp		Trans-GUID86 in Trans-Tech-Soft
	MERGE (grp23)-[:GROUP_HAS_COMPETENCIES_OF {order: 117, planId: "2"}]->(cmp86:Competency {id: "Trans-GUID86"})-[:IS_IN_GROUP {order: 117, planId: "2"}]->(grp23)
	SET cmp86.label = "Help team with first aid on software",
		cmp86.default_weight = 1.0
	MERGE (cmp86)-[:HAS_SHORT_NAME]->(sn117:ShortName {id : "sn117"})
	SET sn117.label = "Help team with first aid on software"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp86)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp86)-[:HAS_TARGET_COMPETENCY]->(tc93:TargetCompetency {id : "tc93"})
	SET tc93.label = "Demonstrate the ability to help translation teams (�first aid�), when they run into a problem with their equipment and software (e.g. reinstalling the relevant software)."
	...
	MERGE (grp24)-[:HAS_SHORT_NAME]->(sn118:ShortName {id : "sn118"})
	SET sn118.label = "B) General"

	//Comp		Trans-GUID87 in Trans-Tech-Gen
	MERGE (grp24)-[:GROUP_HAS_COMPETENCIES_OF {order: 119, planId: "2"}]->(cmp87:Competency {id: "Trans-GUID87"})-[:IS_IN_GROUP {order: 119, planId: "2"}]->(grp24)
	SET cmp87.label = "Maintain technological devices",
		cmp87.default_weight = 1.0
	MERGE (cmp87)-[:HAS_SHORT_NAME]->(sn119:ShortName {id : "sn119"})
	SET sn119.label = "Maintain technological devices"
	...
	MERGE (grp24)-[:GROUP_HAS_COMPETENCIES_OF {order: 120, planId: "2"}]->(cmp88:Competency {id: "Trans-GUID88"})-[:IS_IN_GROUP {order: 120, planId: "2"}]->(grp24)
	SET cmp88.label = "Safeguard data",
		cmp88.default_weight = 1.0
	MERGE (cmp88)-[:HAS_SHORT_NAME]->(sn120:ShortName {id : "sn120"})
	SET sn120.label = "Safeguard data"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp88)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp88)-[:HAS_TARGET_COMPETENCY]->(tc95:TargetCompetency {id : "tc95"})
	SET tc95.label = "Keep data safe and well-organized."
	...
	MERGE (grp24)-[:GROUP_HAS_COMPETENCIES_OF {order: 121, planId: "2"}]->(cmp89:Competency {id: "Trans-GUID89"})-[:IS_IN_GROUP {order: 121, planId: "2"}]->(grp24)
	SET cmp89.label = "Digital communication",
		cmp89.default_weight = 1.0
	MERGE (cmp89)-[:HAS_SHORT_NAME]->(sn121:ShortName {id : "sn121"})
	SET sn121.label = "Digital communication"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp89)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp89)-[:HAS_TARGET_COMPETENCY]->(tc96:TargetCompetency {id : "tc96"})
	SET tc96.label = "Use various devices and digital tools to communicate with others."
	...
	MERGE (grp24)-[:GROUP_HAS_COMPETENCIES_OF {order: 122, planId: "2"}]->(cmp90:Competency {id: "Trans-GUID90"})-[:IS_IN_GROUP {order: 122, planId: "2"}]->(grp24)
	SET cmp90.label = "Written communication",
		cmp90.default_weight = 1.0
	MERGE (cmp90)-[:HAS_SHORT_NAME]->(sn122:ShortName {id : "sn122"})
	SET sn122.label = "Written communication"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp90)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp90)-[:HAS_TARGET_COMPETENCY]->(tc97:TargetCompetency {id : "tc97"})
	SET tc97.label = "Use best practices for email and other written communication with others."
	...
	MERGE (grp24)-[:GROUP_HAS_COMPETENCIES_OF {order: 123, planId: "2"}]->(cmp91:Competency {id: "Trans-GUID91"})-[:IS_IN_GROUP {order: 123, planId: "2"}]->(grp24)
	SET cmp91.label = "Collaboration tools",
		cmp91.default_weight = 1.0
	MERGE (cmp91)-[:HAS_SHORT_NAME]->(sn123:ShortName {id : "sn123"})
	SET sn123.label = "Collaboration tools"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp91)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp91)-[:HAS_TARGET_COMPETENCY]->(tc98:TargetCompetency {id : "tc98"})
	SET tc98.label = "Use collaboration tools (e.g. file sharing, track changes, Google, etc.)."
	...
	//Comp		Trans-GUID92 in Trans-Tech-Gen
	MERGE (grp24)-[:GROUP_HAS_COMPETENCIES_OF {order: 124, planId: "2"}]->(cmp92:Competency {id: "Trans-GUID92"})-[:IS_IN_GROUP {order: 124, planId: "2"}]->(grp24)
	SET cmp92.label = "Internet",
		cmp92.default_weight = 1.0
	MERGE (cmp92)-[:HAS_SHORT_NAME]->(sn124:ShortName {id : "sn124"})
	SET sn124.label = "Internet"
...
`);
	...
	MERGE (grp25)-[:HAS_SHORT_NAME]->(sn126:ShortName {id : "sn126"})
	SET sn126.label = "A) General elements of program design"

	//Comp		Trans-GUID93 in Trans-Prog-Gen
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 127, planId: "2"}]->(cmp93:Competency {id: "Trans-GUID93"})-[:IS_IN_GROUP {order: 127, planId: "2"}]->(grp25)
	SET cmp93.label = "Planning process",
		cmp93.default_weight = 1.0
	MERGE (cmp93)-[:HAS_SHORT_NAME]->(sn127:ShortName {id : "sn127"})
	SET sn127.label = "Planning process"
	...
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 128, planId: "2"}]->(cmp94:Competency {id: "Trans-GUID94"})-[:IS_IN_GROUP {order: 128, planId: "2"}]->(grp25)
	SET cmp94.label = "Project funding proposal",
		cmp94.default_weight = 1.0
	MERGE (cmp94)-[:HAS_SHORT_NAME]->(sn128:ShortName {id : "sn128"})
	SET sn128.label = "Project funding proposal"
	...
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 129, planId: "2"}]->(cmp95:Competency {id: "Trans-GUID95"})-[:IS_IN_GROUP {order: 129, planId: "2"}]->(grp25)
	SET cmp95.label = "Authorisations required",
		cmp95.default_weight = 1.0
	MERGE (cmp95)-[:HAS_SHORT_NAME]->(sn129:ShortName {id : "sn129"})
	SET sn129.label = "Authorisations required"
	...
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 130, planId: "2"}]->(cmp96:Competency {id: "Trans-GUID96"})-[:IS_IN_GROUP {order: 130, planId: "2"}]->(grp25)
	SET cmp96.label = "Sociolingiustic relevance",
		cmp96.default_weight = 1.0
	MERGE (cmp96)-[:HAS_SHORT_NAME]->(sn130:ShortName {id : "sn130"})
	SET sn130.label = "Sociolingiustic relevance"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp96)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp96)-[:HAS_TARGET_COMPETENCY]->(tc104:TargetCompetency {id : "tc104"})
	SET tc104.label = "Explain the relevance of basic concepts of sociolinguistics, such as variation, identity, multilingualism, attitudes, vitality, etc. to language program design."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID96"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp96)
	Merge (cmp96)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms)
	Merge (cmp96)-[:TARGET_VALUE_IS {planId: "2", min: 1}]->(ms2)

	//Comp		Trans-GUID97 in Trans-Prog-Gen
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 131, planId: "2"}]->(cmp97:Competency {id: "Trans-GUID97"})-[:IS_IN_GROUP {order: 131, planId: "2"}]->(grp25)
	SET cmp97.label = "Intellectual property",
		cmp97.default_weight = 1.0
	MERGE (cmp97)-[:HAS_SHORT_NAME]->(sn131:ShortName {id : "sn131"})
	SET sn131.label = "Intellectual property"
	...
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 132, planId: "2"}]->(cmp98:Competency {id: "Trans-GUID98"})-[:IS_IN_GROUP {order: 132, planId: "2"}]->(grp25)
	SET cmp98.label = "Working in partnership",
		cmp98.default_weight = 1.0
	MERGE (cmp98)-[:HAS_SHORT_NAME]->(sn132:ShortName {id : "sn132"})
	SET sn132.label = "Working in partnership"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp98)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp98)-[:HAS_TARGET_COMPETENCY]->(tc106:TargetCompetency {id : "tc106"})
	SET tc106.label = "Demonstrate understanding of, and positive attitudes towards, working through partnerships and teams to accomplish all language development strategic goals."
	...
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 133, planId: "2"}]->(cmp99:Competency {id: "Trans-GUID99"})-[:IS_IN_GROUP {order: 133, planId: "2"}]->(grp25)
	SET cmp99.label = "Participatory methods",
		cmp99.default_weight = 1.0
	MERGE (cmp99)-[:HAS_SHORT_NAME]->(sn133:ShortName {id : "sn133"})
	SET sn133.label = "Participatory methods"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp99)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp99)-[:HAS_TARGET_COMPETENCY]->(tc107:TargetCompetency {id : "tc107"})
	SET tc107.label = "Demonstrate an ability to use participatory methods when working with a language community to promote acceptance."
	...
	//Comp		Trans-GUID100 in Trans-Prog-Gen
	MERGE (grp25)-[:GROUP_HAS_COMPETENCIES_OF {order: 134, planId: "2"}]->(cmp100:Competency {id: "Trans-GUID100"})-[:IS_IN_GROUP {order: 134, planId: "2"}]->(grp25)
	SET cmp100.label = "Best practices for project design",
		cmp100.default_weight = 1.0
	MERGE (cmp100)-[:HAS_SHORT_NAME]->(sn134:ShortName {id : "sn134"})
	SET sn134.label = "Best practices for project design"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp100)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp100)-[:HAS_TARGET_COMPETENCY]->(tc108:TargetCompetency {id : "tc108"})
	SET tc108.label = "Explain general principles, approaches and best practices for designing and developing language programs. Understands the interplay between Budget, Time, Scope, project goals, and their impact on Quality and can successfully shape and navigate these things to meet desired parameters."
	...
	MERGE (grp26)-[:HAS_SHORT_NAME]->(sn135:ShortName {id : "sn135"})
	SET sn135.label = "B) Roles of consultants within a BT/SE program"

	//Comp		Trans-GUID101 in Trans-Prog-Role
	MERGE (grp26)-[:GROUP_HAS_COMPETENCIES_OF {order: 136, planId: "2"}]->(cmp101:Competency {id: "Trans-GUID101"})-[:IS_IN_GROUP {order: 136, planId: "2"}]->(grp26)
	SET cmp101.label = "Interdisciplinary approach",
		cmp101.default_weight = 1.0
	MERGE (cmp101)-[:HAS_SHORT_NAME]->(sn136:ShortName {id : "sn136"})
	SET sn136.label = "Interdisciplinary approach"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp101)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp101)-[:HAS_TARGET_COMPETENCY]->(tc109:TargetCompetency {id : "tc109"})
	SET tc109.label = "Articulate the roles of the different academic domains in a language program."
	...
	MERGE (grp26)-[:GROUP_HAS_COMPETENCIES_OF {order: 137, planId: "2"}]->(cmp102:Competency {id: "Trans-GUID102"})-[:IS_IN_GROUP {order: 137, planId: "2"}]->(grp26)
	SET cmp102.label = "Scripture Engagement principles",
		cmp102.default_weight = 1.0
	MERGE (cmp102)-[:HAS_SHORT_NAME]->(sn137:ShortName {id : "sn137"})
	SET sn137.label = "Scripture Engagement principles"
	...
	MERGE (grp26)-[:GROUP_HAS_COMPETENCIES_OF {order: 138, planId: "2"}]->(cmp103:Competency {id: "Trans-GUID103"})-[:IS_IN_GROUP {order: 138, planId: "2"}]->(grp26)
	SET cmp103.label = "Multilingualism Urbanisation & Scripture Engagement (MUSE)",
		cmp103.default_weight = 1.0
	MERGE (cmp103)-[:HAS_SHORT_NAME]->(sn138:ShortName {id : "sn138"})
	SET sn138.label = "Multilingualism Urbanisation & Scripture Engagement (MUSE)"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp103)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp103)-[:HAS_TARGET_COMPETENCY]->(tc111:TargetCompetency {id : "tc111"})
	SET tc111.label = "Articulate the relevance of multilingualism, urbanization and migration to a BT/SE program."
	...
	MERGE (grp26)-[:GROUP_HAS_COMPETENCIES_OF {order: 139, planId: "2"}]->(cmp104:Competency {id: "Trans-GUID104"})-[:IS_IN_GROUP {order: 139, planId: "2"}]->(grp26)
	SET cmp104.label = "Church mission & community engagement",
		cmp104.default_weight = 1.0
	MERGE (cmp104)-[:HAS_SHORT_NAME]->(sn139:ShortName {id : "sn139"})
	SET sn139.label = "Church mission & community engagement"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp104)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp104)-[:HAS_TARGET_COMPETENCY]->(tc112:TargetCompetency {id : "tc112"})
	SET tc112.label = "Articulate and appreciate the role of the church and missions in a BT/SE program: how the community will engage with the Scriptures and the implications that has for how Scripture is translated."
	...
	//Comp		Trans-GUID105 in Trans-Prog-Role
	MERGE (grp26)-[:GROUP_HAS_COMPETENCIES_OF {order: 140, planId: "2"}]->(cmp105:Competency {id: "Trans-GUID105"})-[:IS_IN_GROUP {order: 140, planId: "2"}]->(grp26)
	SET cmp105.label = "SE approaches",
		cmp105.default_weight = 1.0
	MERGE (cmp105)-[:HAS_SHORT_NAME]->(sn140:ShortName {id : "sn140"})
	SET sn140.label = "SE approaches"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp105)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp105)-[:HAS_TARGET_COMPETENCY]->(tc113:TargetCompetency {id : "tc113"})
	SET tc113.label = "Is familiar with a variety of SE approaches (e.g. private reading, public reading, Storying Fellowship Groups, Listening Groups, apps). Demonstrate a good understanding of the most relevant developments and approaches in Scripture Engagement. "
...
`);
	...
	MERGE (grp27)-[:HAS_SHORT_NAME]->(sn142:ShortName {id : "sn142"})
	SET sn142.label = "A) Holistic personal growth"

	//Comp		Trans-GUID106 in Trans-Interp-Hol
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 143, planId: "2"}]->(cmp106:Competency {id: "Trans-GUID106"})-[:IS_IN_GROUP {order: 143, planId: "2"}]->(grp27)
	SET cmp106.label = "Awareness of culture",
		cmp106.default_weight = 1.0
	MERGE (cmp106)-[:HAS_SHORT_NAME]->(sn143:ShortName {id : "sn143"})
	SET sn143.label = "Awareness of culture"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp106)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp106)-[:HAS_TARGET_COMPETENCY]->(tc115:TargetCompetency {id : "tc115"})
	SET tc115.label = "Is able to explain how one�s own cultural background and personality impacts one's thinking, values and behavior and how one relates to others."
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 144, planId: "2"}]->(cmp107:Competency {id: "Trans-GUID107"})-[:IS_IN_GROUP {order: 144, planId: "2"}]->(grp27)
	SET cmp107.label = "Self-awareness",
		cmp107.default_weight = 1.0
	MERGE (cmp107)-[:HAS_SHORT_NAME]->(sn144:ShortName {id : "sn144"})
	SET sn144.label = "Self-awareness"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp107)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp107)-[:HAS_TARGET_COMPETENCY]->(tc116:TargetCompetency {id : "tc116"})
	SET tc116.label = "Being aware of one's own personality and cultural background, is able to take responsible courses of action in cross-cultural situations."
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 145, planId: "2"}]->(cmp108:Competency {id: "Trans-GUID108"})-[:IS_IN_GROUP {order: 145, planId: "2"}]->(grp27)
	SET cmp108.label = "Tolerance",
		cmp108.default_weight = 1.0
	MERGE (cmp108)-[:HAS_SHORT_NAME]->(sn145:ShortName {id : "sn145"})
	SET sn145.label = "Tolerance"
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 146, planId: "2"}]->(cmp109:Competency {id: "Trans-GUID109"})-[:IS_IN_GROUP {order: 146, planId: "2"}]->(grp27)
	SET cmp109.label = "Acknowledging responsibility",
		cmp109.default_weight = 1.0
	MERGE (cmp109)-[:HAS_SHORT_NAME]->(sn146:ShortName {id : "sn146"})
	SET sn146.label = "Acknowledging responsibility"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp109)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp109)-[:HAS_TARGET_COMPETENCY]->(tc118:TargetCompetency {id : "tc118"})
	SET tc118.label = "Is willing to take responsibility for negative patterns that affect one's interactions in community."
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 147, planId: "2"}]->(cmp110:Competency {id: "Trans-GUID110"})-[:IS_IN_GROUP {order: 147, planId: "2"}]->(grp27)
	SET cmp110.label = "Initiative",
		cmp110.default_weight = 1.0
	MERGE (cmp110)-[:HAS_SHORT_NAME]->(sn147:ShortName {id : "sn147"})
	SET sn147.label = "Initiative"
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 148, planId: "2"}]->(cmp111:Competency {id: "Trans-GUID111"})-[:IS_IN_GROUP {order: 148, planId: "2"}]->(grp27)
	SET cmp111.label = "Self-control",
		cmp111.default_weight = 1.0
	MERGE (cmp111)-[:HAS_SHORT_NAME]->(sn148:ShortName {id : "sn148"})
	SET sn148.label = "Self-control"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp111)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp111)-[:HAS_TARGET_COMPETENCY]->(tc120:TargetCompetency {id : "tc120"})
	SET tc120.label = "Is intentional about dealing with one's own inner dynamics appropriately as one becomes aware of them."
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 149, planId: "2"}]->(cmp112:Competency {id: "Trans-GUID112"})-[:IS_IN_GROUP {order: 149, planId: "2"}]->(grp27)
	SET cmp112.label = "Balance",
		cmp112.default_weight = 1.0
	MERGE (cmp112)-[:HAS_SHORT_NAME]->(sn149:ShortName {id : "sn149"})
	SET sn149.label = "Balance"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp112)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp112)-[:HAS_TARGET_COMPETENCY]->(tc121:TargetCompetency {id : "tc121"})
	SET tc121.label = "Balances workload in a healthy way."
	...
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 150, planId: "2"}]->(cmp113:Competency {id: "Trans-GUID113"})-[:IS_IN_GROUP {order: 150, planId: "2"}]->(grp27)
	SET cmp113.label = "Dependability",
		cmp113.default_weight = 1.0
	MERGE (cmp113)-[:HAS_SHORT_NAME]->(sn150:ShortName {id : "sn150"})
	SET sn150.label = "Dependability"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp113)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp113)-[:HAS_TARGET_COMPETENCY]->(tc122:TargetCompetency {id : "tc122"})
	SET tc122.label = "Fulfills obligations/achieves agreed upon goals consistently."
	...
	//Comp		Trans-GUID114 in Trans-Interp-Hol
	MERGE (grp27)-[:GROUP_HAS_COMPETENCIES_OF {order: 151, planId: "2"}]->(cmp114:Competency {id: "Trans-GUID114"})-[:IS_IN_GROUP {order: 151, planId: "2"}]->(grp27)
	SET cmp114.label = "Attitude",
		cmp114.default_weight = 1.0
	MERGE (cmp114)-[:HAS_SHORT_NAME]->(sn151:ShortName {id : "sn151"})
	SET sn151.label = "Attitude"
	...
	MERGE (grp28)-[:HAS_SHORT_NAME]->(sn152:ShortName {id : "sn152"})
	SET sn152.label = "B) Interpersonal relationships"

	//Comp		Trans-GUID115 in Trans-Interp-Rel
	MERGE (grp28)-[:GROUP_HAS_COMPETENCIES_OF {order: 153, planId: "2"}]->(cmp115:Competency {id: "Trans-GUID115"})-[:IS_IN_GROUP {order: 153, planId: "2"}]->(grp28)
	SET cmp115.label = "Even-tempered",
		cmp115.default_weight = 1.0
	MERGE (cmp115)-[:HAS_SHORT_NAME]->(sn153:ShortName {id : "sn153"})
	SET sn153.label = "Even-tempered"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp115)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp115)-[:HAS_TARGET_COMPETENCY]->(tc124:TargetCompetency {id : "tc124"})
	SET tc124.label = "Takes responsibility for negative patterns, does not easily take offense."
	...
	MERGE (grp28)-[:GROUP_HAS_COMPETENCIES_OF {order: 154, planId: "2"}]->(cmp116:Competency {id: "Trans-GUID116"})-[:IS_IN_GROUP {order: 154, planId: "2"}]->(grp28)
	SET cmp116.label = "Builds relationships",
		cmp116.default_weight = 1.0
	MERGE (cmp116)-[:HAS_SHORT_NAME]->(sn154:ShortName {id : "sn154"})
	SET sn154.label = "Builds relationships"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp116)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp116)-[:HAS_TARGET_COMPETENCY]->(tc125:TargetCompetency {id : "tc125"})
	SET tc125.label = "Builds personal relationships that extend beyond work situations."
	...
	MERGE (grp28)-[:GROUP_HAS_COMPETENCIES_OF {order: 155, planId: "2"}]->(cmp117:Competency {id: "Trans-GUID117"})-[:IS_IN_GROUP {order: 155, planId: "2"}]->(grp28)
	SET cmp117.label = "Genuine interest",
		cmp117.default_weight = 1.0
	MERGE (cmp117)-[:HAS_SHORT_NAME]->(sn155:ShortName {id : "sn155"})
	SET sn155.label = "Genuine interest"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp117)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp117)-[:HAS_TARGET_COMPETENCY]->(tc126:TargetCompetency {id : "tc126"})
	SET tc126.label = "Shows genuine interest in the work of others."
	...
	MERGE (grp28)-[:GROUP_HAS_COMPETENCIES_OF {order: 156, planId: "2"}]->(cmp118:Competency {id: "Trans-GUID118"})-[:IS_IN_GROUP {order: 156, planId: "2"}]->(grp28)
	SET cmp118.label = "Constructive critcism",
		cmp118.default_weight = 1.0
	MERGE (cmp118)-[:HAS_SHORT_NAME]->(sn156:ShortName {id : "sn156"})
	SET sn156.label = "Constructive critcism"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp118)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp118)-[:HAS_TARGET_COMPETENCY]->(tc127:TargetCompetency {id : "tc127"})
	SET tc127.label = "Expresses and receives criticism in constructive ways."
	...
	//Comp		Trans-GUID119 in Trans-Interp-Rel
	MERGE (grp28)-[:GROUP_HAS_COMPETENCIES_OF {order: 157, planId: "2"}]->(cmp119:Competency {id: "Trans-GUID119"})-[:IS_IN_GROUP {order: 157, planId: "2"}]->(grp28)
	SET cmp119.label = "Learning spirit",
		cmp119.default_weight = 1.0
	MERGE (cmp119)-[:HAS_SHORT_NAME]->(sn157:ShortName {id : "sn157"})
	SET sn157.label = "Learning spirit"
	...
	MERGE (grp29)-[:HAS_SHORT_NAME]->(sn158:ShortName {id : "sn158"})
	SET sn158.label = "C) Skills in consulting situation"

	//Comp		Trans-GUID120 in Trans-Interp-Situ
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 159, planId: "2"}]->(cmp120:Competency {id: "Trans-GUID120"})-[:IS_IN_GROUP {order: 159, planId: "2"}]->(grp29)
	SET cmp120.label = "Respect",
		cmp120.default_weight = 1.0
	MERGE (cmp120)-[:HAS_SHORT_NAME]->(sn159:ShortName {id : "sn159"})
	SET sn159.label = "Respect"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp120)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp120)-[:HAS_TARGET_COMPETENCY]->(tc129:TargetCompetency {id : "tc129"})
	SET tc129.label = "Shows respect for the person consulted: that s/he is a learner but also an expert in his/her own domain."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID120"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp120)
	Merge (cmp120)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms)
	Merge (cmp120)-[:TARGET_VALUE_IS {planId: "2", min: 1}]->(ms2)

	//Comp		Trans-GUID121 in Trans-Interp-Situ
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 160, planId: "2"}]->(cmp121:Competency {id: "Trans-GUID121"})-[:IS_IN_GROUP {order: 160, planId: "2"}]->(grp29)
	SET cmp121.label = "Listens to needs",
		cmp121.default_weight = 1.0
	MERGE (cmp121)-[:HAS_SHORT_NAME]->(sn160:ShortName {id : "sn160"})
	SET sn160.label = "Listens to needs"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp121)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp121)-[:HAS_TARGET_COMPETENCY]->(tc130:TargetCompetency {id : "tc130"})
	SET tc130.label = "Listens attentively to understand what others need and want."
	...
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 161, planId: "2"}]->(cmp122:Competency {id: "Trans-GUID122"})-[:IS_IN_GROUP {order: 161, planId: "2"}]->(grp29)
	SET cmp122.label = "Insightful questions",
		cmp122.default_weight = 1.0
	MERGE (cmp122)-[:HAS_SHORT_NAME]->(sn161:ShortName {id : "sn161"})
	SET sn161.label = "Insightful questions"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp122)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp122)-[:HAS_TARGET_COMPETENCY]->(tc131:TargetCompetency {id : "tc131"})
	SET tc131.label = "Formulates insightful questions."
	...
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 162, planId: "2"}]->(cmp123:Competency {id: "Trans-GUID123"})-[:IS_IN_GROUP {order: 162, planId: "2"}]->(grp29)
	SET cmp123.label = "Responsive",
		cmp123.default_weight = 1.0
	MERGE (cmp123)-[:HAS_SHORT_NAME]->(sn162:ShortName {id : "sn162"})
	SET sn162.label = "Responsive"
	...
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 163, planId: "2"}]->(cmp124:Competency {id: "Trans-GUID124"})-[:IS_IN_GROUP {order: 163, planId: "2"}]->(grp29)
	SET cmp124.label = "Gives advice",
		cmp124.default_weight = 1.0
	MERGE (cmp124)-[:HAS_SHORT_NAME]->(sn163:ShortName {id : "sn163"})
	SET sn163.label = "Gives advice"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp124)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp124)-[:HAS_TARGET_COMPETENCY]->(tc133:TargetCompetency {id : "tc133"})
	SET tc133.label = "Gives advice in a way that is appropriate and persuasive."
	...
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 164, planId: "2"}]->(cmp125:Competency {id: "Trans-GUID125"})-[:IS_IN_GROUP {order: 164, planId: "2"}]->(grp29)
	SET cmp125.label = "Gives affirmation",
		cmp125.default_weight = 1.0
	MERGE (cmp125)-[:HAS_SHORT_NAME]->(sn164:ShortName {id : "sn164"})
	SET sn164.label = "Gives affirmation"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp125)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp125)-[:HAS_TARGET_COMPETENCY]->(tc134:TargetCompetency {id : "tc134"})
	SET tc134.label = "Expresses appreciation and affirmation for good work and goals met, and so motivates those being consulted."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID125"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp125)
	Merge (cmp125)-[:TARGET_VALUE_IS {planId: "2", min: 3}]->(ms)
	Merge (cmp125)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms2)

	//Comp		Trans-GUID126 in Trans-Interp-Situ
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 165, planId: "2"}]->(cmp126:Competency {id: "Trans-GUID126"})-[:IS_IN_GROUP {order: 165, planId: "2"}]->(grp29)
	SET cmp126.label = "Diffuses tension",
		cmp126.default_weight = 1.0
	MERGE (cmp126)-[:HAS_SHORT_NAME]->(sn165:ShortName {id : "sn165"})
	SET sn165.label = "Diffuses tension"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp126)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp126)-[:HAS_TARGET_COMPETENCY]->(tc135:TargetCompetency {id : "tc135"})
	SET tc135.label = "Seeks to defuse tension and promote positive relationships in culturally appropriate ways."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID126"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp126)
	Merge (cmp126)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms)
	Merge (cmp126)-[:TARGET_VALUE_IS {planId: "2", min: 1}]->(ms2)

	//Comp		Trans-GUID127 in Trans-Interp-Situ
	MERGE (grp29)-[:GROUP_HAS_COMPETENCIES_OF {order: 166, planId: "2"}]->(cmp127:Competency {id: "Trans-GUID127"})-[:IS_IN_GROUP {order: 166, planId: "2"}]->(grp29)
	SET cmp127.label = "Resolves conflicts",
		cmp127.default_weight = 1.0
	MERGE (cmp127)-[:HAS_SHORT_NAME]->(sn166:ShortName {id : "sn166"})
	SET sn166.label = "Resolves conflicts"
	MERGE (d)-[:PRIMARY_DOMAIN_OF]->(cmp127)-[:HAS_PRIMARY_DOMAIN]->(d)
	MERGE (cmp127)-[:HAS_TARGET_COMPETENCY]->(tc136:TargetCompetency {id : "tc136"})
	SET tc136.label = "Resolves conflicts in a way that results in strengthened, positive relationships. (for example with regard to the question of how to translate a certain Biblical key term), or when there is a conflict between the translation team and the translation consultant."
	Merge (pr)-[:CHILD_PROGRESS]->(:Progress {currentLevel: 2, competency_ref: "Trans-GUID127"})-[:COMPETENCY_PROGRESS {userId: "1"}]-(cmp127)
	Merge (cmp127)-[:TARGET_VALUE_IS {planId: "2", min: 3}]->(ms)
	Merge (cmp127)-[:TARGET_VALUE_IS {planId: "2", min: 2}]->(ms2)
